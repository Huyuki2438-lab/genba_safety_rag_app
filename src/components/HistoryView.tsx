...
export function HistoryView({ onOpenKy, onSelect }: Props) {
  ...
  const load = async (next = filters) => { try { setMessage(""); setEntries(await fetchAnalysisHistory(next)); } catch (error) { setMessage(error instanceof Error ? error.message : "共有データへ接続できません。"); } };
  ...
  return <main className="app-main history-page">
    ...
    <section className="history-filters"><h2>履歴</h2><div className="filter-grid">
    ...
    <section className="history-table-wrap"><table className="history-table"><thead><tr><th>日時</th><th>写真</th><th>現場名</th><th>作業内容</th><th>主な危険</th><th>登録者</th></tr></thead><tbody>
      {entries.map((entry) => <tr key={entry.id} onClick={() => setSelected(entry)}><td>{formatDate(entry.createdAt)}</td><td>{entry.imageUrl && <img src={entry.imageUrl} alt="" />}</td><td>{entry.siteName}</td><td>{entry.workContent}</td><td>{entry.mainRisk}</td><td>{entry.createdBy}</td></tr>)}
      {!entries.length && <tr><td colSpan={6}>該当する履歴はありません。</td></tr>}</tbody></table></section>
    {selected && <section className="history-detail"><div className="detail-actions"><h2>履歴詳細</h2><button type="button" className="secondary-button compact" onClick={() => { onSelect(selected); onOpenKy(); }}>KY作成画面で再表示</button><button type="button" className="delete-button" onClick={() => void remove()}>削除</button></div>
