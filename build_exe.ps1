...
if (Test-Path (Join-Path $root "dist\GenbaSafetyRAGApp")) { Remove-Item -Recurse -Force (Join-Path $root "dist\GenbaSafetyRAGApp") }
& $venvPython -m PyInstaller desktop_app.spec --noconfirm
if ($LASTEXITCODE -ne 0) { throw "PyInstaller に失敗しました" }

$outDir = Join-Path $root "dist\GenbaSafetyRAGApp"
...
Copy-Item -Recurse -Force (Join-Path $root "static") (Join-Path $outDir "static")
Copy-Item -Recurse -Force (Join-Path $root "templates") (Join-Path $outDir "templates")
