...
import { createAnalysisHistory, fetchAnalysisHistory } from "../lib/analysisHistoryApi";
...
export const useGeminiSafetyAnalysis = (mode: AiMode) => {
  ...
  const showHistoryEntry = useCallback((entry: AnalysisHistoryEntry) => { setErrorMessage(null); setActiveHistoryId(entry.id); setAnalysisMarkdown(entry.markdown); }, []);
  const analyzeSelectedImage = useCallback(async (selectedImage: File | null, metadata: KyMetadata): Promise<void> => {
    ...
    setIsAnalyzing(true); setErrorMessage(null); setAnalysisMarkdown(""); setActiveHistoryId(null);
    try {
      const result = await analyzeSafetyImage({ imageFile: selectedImage, mode });
      const saved = await createAnalysisHistory({ imageName: selectedImage.name || "現場写真", imageMimeType: getImageMimeType(selectedImage), imageBase64: await fileToBase64(selectedImage), mode, providerDisplayLabel: runtimeConfig.providerDisplayLabel, model: runtimeConfig.model, markdown: result.markdown, ...metadata });
  ...
  return { isAnalyzing, analysisMarkdown, analysisHistory, activeHistoryId, activeHistoryEntry, errorMessage, analyzeSelectedImage, runtimeConfig, showHistoryEntry, reloadHistory };
};
